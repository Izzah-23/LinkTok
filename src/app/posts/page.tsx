...
import { Heart, MessageCircle, Share2, Share2Icon } from "lucide-react";
...
console.log(shareData);
  ...
  return (
    <>
    <div className="  min-h-screen bg-gradient-to-br from-red-500 to-yellow-100 ">
    <div className="grid grid-cols-3  grid-flow-row-1 gap-4 mx-16 ">
   
  
      {isLoading && <Loader />}
        {data?.followingPost.map((post: any) => (
          <div className="relative " >
          <div key={post.post_id} className="bg-neutral-300 mt-8 rounded-lg">
            <div className=" grid grid-cols-2  rounded-lg p-2 mb-2 ">
              <img
                src={post.profilePictureUrl}
                alt={`${post.username}'s Profile`}
                className="h-16 w-16 rounded-full"
              />
            <h3 className="text-xl font-bold -ml-24 mt-4">{post.username}</h3>
  ...
  <div className="absolute top-12 right-6 mt-2 ml-2  ">
  <Popover>
    <PopoverTrigger>
      ...
      <div >
        <Dialog >
          ...
          <DialogContent>
            ...
            <div className="space-x-1 flex items-center mx-4 mt-4 ">
              <Button onClick={() => handleLike(post.post_id)} className="mb-5">
                <Heart className="mr-2 fill-red-600" />
                {post.likes} Like
              </Button>
...
<Dialog >
  <DialogTrigger asChild>
   ...
   className="mb-5">
      <MessageCircle className="mr-1 ml-1" /> {post.comments} Comments
    </Button>
  </DialogTrigger>
...
<Popover >
  <PopoverTrigger asChild>
   ...
   className="mb-5"> <Share2 className="mr-2 size-2"/> {post.shares} share </Button>
  </PopoverTrigger>
